...
- **A delegated leaf never stages, commits, pushes, or deploys.** It leaves its work unstaged in
  the tree and hands the diff plus proof to the orchestrator, which reviews and commits under its
  own committer name. An interactive or orchestrating Codex session instead follows
  `~/.claude/references/codex-interactive-workflow.md`, including its scoped automatic commit of
  verified task-owned work; pushing still needs Felix's explicit request.
