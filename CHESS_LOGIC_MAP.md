...
5. **Hoher IPC-Traffic (Frontend-Streaming)**:
   Das Senden ganzer Zughistorien (`moves: Vec<GameMove>`) im `GameMoveEvent` bei *jedem* Zug skaliert schlecht für sehr lange Partien, da die JSON-Payload stetig wächst. Uhren-Updates via Tauri-Events können zudem den IPC-Kanal verstopfen, wenn sie nicht gedrosselt (debounced/throttled) werden.
