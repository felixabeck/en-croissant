...
* **Annotated 2026-09-14 (slice 1 landed; drain session 2145a459-9087-4fc6-b86e-a21a0f6995c3, Claude Code build run, executor Codex; status stays open):** the non-Linux targets are now observable and compile locally. Commits: `0f5fb28f` macOS port of the Unix filesystem primitives (one `raw_stat_identity`, checked `raw_mode_from`, one `rustix::fs::Dir` walker shared by listing, install sync and recursive delete with the depth cap on both walks, `f_mntonname` mount evidence plus union-mount refusal on non-Linux Unix; Linux statx/fdinfo unchanged); `bcac14f2` Windows compile surface and typed refusals (`src-tauri/src/platform_support.rs` single refusal source, first-statement guards on the eight external-effect commands and four typed-first sites, whole-function non-Unix counterparts, `IO_STATUS_BLOCK` import fix, Unix-only items cfg-gated); `16476781` CI jobs `rust-platform` (check + clippy -D warnings for aarch64-apple-darwin, x86_64-apple-darwin, x86_64-pc-windows-msvc) and `rust-macos-test` (routed Rust suite on macos-latest) with a target-coverage contract in `scripts/check-tool-version-parity.mjs`; review repairs `b35bf906` (restores the Windows no-delete engine handles the dead-code cleanup had dropped; per-entry install depth) and `a4b6b3ce`. Decisions `d-20260914-05`..`-08`. Plan-review record (20 rounds, R1-01..R20-01): `tasks/handoffs/2026-09-14-f-20260830-06-slice-1-review.md`. Local proof: Linux fmt/check/clippy/1121 tests/bindings; aarch64-apple-darwin and x86_64-pc-windows-gnu check + clippy -D warnings via a local zig cross toolchain (advisory only — no macOS or Windows runtime was exercised locally; the real-runner jobs are the authority and first run on this push). Still owed by this finding, filed as follow-ups with Root `non-linux-platform-port` (inbox spool, ids allocated at merge): (a) Windows test job `20260914-053235-3129284`, (b) Windows workspace mutations `053235-3129311`, (c) Windows database/search-index/puzzle operations `053235-3129333` — `get_puzzle` is refused, `get_puzzle_db_info`/`get_puzzle_themes`/`get_themes_for_puzzle` are not, (d) atomic replacement/downloads/PGN writes `053235-3129769`, (e) startup registries and app-owned roots `053235-3130083` — credential refusals currently surface as the redacted `CredentialFailure`, (f) engine directory resources/archive install/executable mode `053235-3130471`, (g) non-Linux sound route `053236-3130491`, (h) refusal ordering after in-process work `070326-3763436`; plus the Windows PGN cache revision signal `20260914-170753-4126311`. Each loads the handoff above before its own plan review.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"8135aa13d0ee39051377e4371810719a05c6d668355d33d0f04c07b1500806a3","input_sha256":"a2daa83a63e73028725dbf01cc4ae4970a8c6335b9cfee8cedf61bcd7139704f","kind":"mutation-receipt","operation":"8304673ac36eadf388ad3da053fe8b2de193396358446de3fb5abf2d460111a5","options":{"section":null},"request_id_sha256":"a3fb3187d9c1f591e0df161883d7d6cdbb0a677049a8b278285552d8b386cf08","results":["f-20260830-06"],"target":"f-20260830-06","v":1} -->
...
* **Resolved, 2026-09-10:** 813563b7 and 2a6f66df add a synchronous save-before-publish workspace receipt. New tab creation and activation share one envelope write; seeded new IDs are rolled back when it is refused, and duplicate creation durably copies pending edits without flushing unrelated trees. Page and menu consumers only navigate after admission. Startup migration remains governed by d-20260901-15; live ordering is d-20260910-10 and dependent deletion compensation is d-20260910-11. Root proof: `pnpm test` 128 files/1114 tests, tsgo and scoped oxfmt green; deliberate removal of rollback fails the regression tests. `pnpm test:e2e:container` passed all 14 tests, including actual renderer write refusal, retry and reload, without snapshot updates. The separate existing-ID import replacement transaction remains open as f-20260910-09. Plan authorship and arbitration shared root context; detection and code used the Codex family.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"2f7ad2fe0aaea1e8ec1a496485303bc1eab51e0f9916a555059367b685022e82","input_sha256":"437b2bcf62f1f46b0b80e5c8d24b6237fb069adf459bb0809d61c9a200e805aa","kind":"mutation-receipt","operation":"3bfeb0824bbdb2cf43fb840b9b9b487d05df201eb2d85d682f8f1910d0dda95c","options":{"section":null},"request_id_sha256":null,"results":["f-20260901-05"],"target":"f-20260901-05","v":1} -->
...
* **Review repairs 2026-09-10:** `review-tests` returned REVISE on the first commit with a blocker: both new fixtures created only untracked files, so `listWorkingTreeFiles`'s tracked query was never exercised, and deleting that query left every test green while the real gates enumerated nothing against a committed tree. Measured by removing the line: two of the six new walker tests go red now, none did before. Fixtures stage a file through a new shared `gitTrack` helper, and `scripts/working-tree-files.test.mjs` now proves the walker's own contract once — tracked, untracked, ignored, symlinked, non-repository, and a failing git invocation — instead of each checker re-deriving it (`review-minimalism`). The JSX checker gained a CLI exit-code test, takes its workspace from the working directory like its siblings, and its exports were renamed `listSourceFiles` / `checkUntranslatedJsx` for the local idiom (`review-code-quality`).
* **Commits:** `63202564`, `7b05f356`, `15f2b4ed`
* **Also fixed in-run:** `review-root-cause` found the same enumeration drift one level up in `scripts/check-skill-bridges.mjs` — skill names came from `readdir`, whose `Dirent.isDirectory()` reflects `lstat`, so a skill shipped as a symlinked directory was invisible to the pairing and line-cap checks. Same area, so handled here rather than filed (rule 4b). See `d-PENDING` below for why the walker alone does not close it.
* **Skipped:** the correctness lens's note that a tracked-but-deleted file now surfaces as `ENOENT` exit 2 instead of being silently absent. That is the deliberate fail-loud contract of `f-20260830-54`, and every sibling checker behaves the same way.
* **Lenses:** `review-correctness` APPROVED, `review-root-cause` APPROVED, `review-code-quality` APPROVED, `review-minimalism` APPROVED, `review-tests` REVISE then repaired.
<!-- ledger-meta {"command":"annotate","effect_lines":5,"effect_sha256":"12ed27cd06df7fd7cae44a7d4ab8207683da0b110a664e9f06fdd4c3d3213177","input_sha256":"82018510b2821a415db87e9352508b8c76366528bf6922ee3ed2e4bb35ed27af","kind":"mutation-receipt","operation":"729f597c0eb471d8b31643427e26ebf8ef4f668bd436e33073c073bfa87e7733","options":{"section":null},"request_id_sha256":null,"results":["f-20260901-16"],"target":"f-20260901-16","v":1} -->
...
* **Handled (2026-09-11):** 6478a731 carries explicit individual resource provenance through option resolution, interactive analysis, reports, and game initialization into the shared actor. Outgoing and echoed transcript copies are redacted before retention; wire values and parser input remain raw. Runtime-lifetime redaction knowledge is deduplicated and bounded at 256 values / 64 KiB, rejecting excess before send. Report re-resolution restores provenance alongside the inherited child value. 42ea40f4 separately fixes the pre-existing report-progress leak on initial resource-resolution failure.
* **Proof:** Engine suite 87 passed, chess selector 41 passed, game suite 51 passed. Real child fixtures read the resource bytes through the actual wire value and query successful production log egress. Negative controls for report provenance restoration, wire-value corruption, progress ordering, cumulative byte budget, longest-first replacement, and collapsed multi-resource provenance failed before restoration. Formatting and diff checks passed. Real-app startup/IPC/shutdown smoke passed; container visual matrix 14 passed with no snapshot changes. Final affected push gates follow on this committed tree.
* **Review disposition:** Correctness, root-cause, tauri-security, engine-protocol, chess-semantics, and IPC-contract lenses found no defects. Accepted minimalism, test, and error-handling repairs are complete and inspected. The exhaustive Truncated match suggestion remains Skip for the reason recorded above. No unresolved Fix remains. Plan authorship and arbitration shared one context; detection ran on the same model family as the code.
...
* **Handled:** 2923ae10 localizes start, move, takeback, abort and resign alerts through semantic identities and finite literal translation keys in all 16 shipped catalogues. Missing local-engine selection has a typed error. Alerts resolve in the current locale; sanitized diagnostics include tab/attempt and admitted game/session identity. 66bda68f shares safe failure normalization/logging with collectSequential and retains both causes when native logging fails.
* **Proof:** Root reran the five focused test files: 133 tests passed. Worker extraction, catalogue check, TypeScript, scoped formatting/lint passed. Temporarily restoring the raw catch behavior produced six failures and restoration returned green. Pinned Playwright container passed all 15 tests, including German rejected starts, two retries, no raw diagnostic in the page, and sanitized native logs; the captured German alert was inspected. Existing screenshot baselines were unchanged.
* **Review:** Seven fresh read-only Codex lenses completed. Three Fix findings were repaired and inspected: shared diagnostic implementation, log correlation identity, and deterministic browser-locale initialization. Plan authorship and arbitration shared the root context; detection ran on the same model family as the code. No deferred or newly filed findings. Governing decision: d-20260911-03; d-20260901-36's conversion/pending-state repair remains intact.
...
* **Final repair proof:** 3ad53878 makes typed-error assertions unconditional and passes full-tree oxlint. 802fb9d8 consolidates shared diagnostics in src/platform/errors.ts and removes the temporary utility module; root reran 200 focused tests and all 15 container browser tests successfully. The additional review-ipc-contract lens approved the cumulative diff and platform repair without findings. Eight lenses total; all three review Fix verdicts resolved. Coverage scope, floors, baselines and screenshot baselines are unchanged.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"5e74d55022a1f4fbff20f9ba0f254371133e652d47ecdc92453ed82ce6a12c16","input_sha256":"b6bc67a2c4bddacad0f02e5cf0423cf604f1243131d82063153dc038082870e5","kind":"mutation-receipt","operation":"bc3b47048da73c943654a74cd35b5eef0d61a0f377185f0f64b45f2586adc044","options":{"section":null},"request_id_sha256":null,"results":["f-20260901-21"],"target":"f-20260901-21","v":1} -->
...
* **Final-gate repair:** 3daa373b moves the unchanged helper to `src/components/engines/engineOptions.ts` and updates all three imports. The first coverage gate found the original utils location unmapped; a trial path-map addition was refused by the scope signature and reverted. d-20260911-05 supersedes the placement in d-20260911-04 using this new evidence. The existing engine-module glob measures the helper without any coverage config, floor or baseline changes.
* **Repair proof:** 131 test files and 1,135 tests passed in `pnpm test:coverage`; the helper measured 100% coverage and `pnpm coverage:frontend:check` passed. Root reran the 36 focused tests and coverage check successfully, and inspected the rename-only repair diff. The same six-lens review remains applicable to the unchanged function body and caller semantics. Final gates are rerun after these records.
<!-- ledger-meta {"command":"annotate","effect_lines":2,"effect_sha256":"7db58b7927eccff0dfd63fdb5863a2ae0c1be1851c998fb8573bc9041e8e7733","input_sha256":"bd4284612b325a2ba079a1612ad8cfcd9ec5997a9643598ff1835b7b349e0b80","kind":"mutation-receipt","operation":"e57b26833824a2027e264aa8d0255999cfeb8f1061c6838599e88a7b61635d75","options":{"section":null},"request_id_sha256":null,"results":["f-20260901-24"],"target":"f-20260901-24","v":1} -->
...
* **ID:** f-20260905-03 · **Status:** handled · **Area:** db-search · **Root:** - · **Entry:** build · **Blocked:** none
...
* **Re-scoped, 2026-09-12 (rule 12a split):** a `build` run planned this finding through five review rounds, one 12a rewrite and five more rounds (`tasks/plans/2026-09-12-repository-identity-key.md`, `## Reviews`) without converging, because the mandate carried two designs. It is split: **Mandate A** (the carrier carries the canonical binding — `DatabaseFileTarget.path`, `canonical_binding`, four-operation `database_file_target`, `resolve_database` returning the target) is filed as its own `native-fs` finding and is a prerequisite of this one. **This finding is Mandate B** and keeps: `open_current` (the pathname probe on the carrier, in `infra/`, with the parent-identity step), `puzzle_database_target`, `EntryKey { identity, parent_identity, path }` (rejected: identity alone — hard-linked pathnames sharing one entry produced a race in every formulation across ten rounds), `sqlite_uri` with `file://<canonical>?mode=rw` (measured: diesel 2.1.4 opens with `SQLITE_OPEN_URI`; `mode=rw` refuses an absent file without creating it), the pool build outside the lock serialized per key with bounded cancellable waits and the cancellation check under the publication lock (f-20260905-04 part two), the probe on every `entry()` and after every hand-out, `DatabaseSchemaIdentity::from_file`, the allowlist shrink for `repository.rs` and `db/mod.rs`. **Retirement keeps today's waiting semantics** (`retire_and_wait_cancellable`, then remove): non-waiting retirement with parking/un-park/lease-drop prune, identity tombstones and multi-entry unwind are recorded in the plan as the design that did not converge, with every race found against it. Nine adjacent findings were filed during the review (checker method-blindness, `convert_pgn` cancellation, registry parent identity, revision lost on eviction, loader sidecar residual, `PuzzleRead` building the RW pool, sidecar-before-primary order, non-unix compile, and Mandate A).
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"887b4266721990b889af52baf3de1e053d9cd70c0d3a9756595acc23a63e1d7e","input_sha256":"b7fa04cf75c9a2ca264948839c72b7c32e72cd89a4437b381800a2e72c6dfe33","kind":"mutation-receipt","operation":"f453af7c5463ebe0ff3f06e0c8f0276fa2fd4631f7e0b5a7ad2269f4a166e570","options":{"section":null},"request_id_sha256":null,"results":["f-20260905-03"],"target":"f-20260905-03","v":1} -->
...
* **Handled 2026-09-14 (Claude Code build run, executor Codex):** `PathAuthority` now has a directory-enumeration capability, and every listing reach that was `file_workspace.rs`'s goes through it (four measured at revalidation, not five). `infra/fs.rs` `read_directory_entries_at` snapshots a held directory fd (`openat(".")`, then a no-follow `statat` per kept name). `CapabilityDirectory` offers `entries`, `open_child_directory` (identity-checked), `confirm_entry` and `open_metadata_sidecar`, which reads the sidecar from the enumerated directory. `collect_tree_entries` walks in two passes, depth-bounded at 64, so a pass-1 refusal registers nothing. `.db3` database and puzzle listings refuse a child whose resolved identity differs from the enumerated one. The `file_workspace.rs` release-surface allowlist is 1 (the create-path `timestamp`, `f-20260913-02`). Commits `65bae849`, `b63813d1` and `b326686e`. Plan and review history: `tasks/plans/2026-09-13-workspace-directory-enumeration.md` (ignored run artefact), with the durable record in `tasks/handoffs/2026-09-13-workspace-directory-enumeration-review.md`. Push review ran correctness, root-cause, tests, error-handling, tauri-security, minimalism and code-quality; the adopted findings were fixed and closure-checked. Successors: `f-20260913-02`..`-06`, `f-20260912-05`, `f-20260912-10` (Windows port).
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"4126085c12b1ed41863577ee15150d94936dfe903f902cd4b6aa545bebacc1a8","input_sha256":"91bb49d893cee159caa501d8cda32c56bf124008983559639bd6f8d6179022fa","kind":"mutation-receipt","operation":"a78b237fb6064466045fea7e56aed32e5059518a71a153d4f8f5c8bb25876f5f","options":{"section":null},"request_id_sha256":null,"results":["f-20260905-05"],"target":"f-20260905-05","v":1} -->
...
* **Annotated 2026-09-14 (macOS engine-launch build run; status stays open):** the plan for `f-20260914-31` adds a fifth app-owned root, `AppOwnedDefaultRoot::EngineLaunch` (leaf `engine-launch`, mode `0o700`), materialised through the same `ensure_app_owned_default_dir` door on Apple targets. It therefore inherits this finding's ancestor-symlink `create_dir_all` window, raised again by the `review-tauri-security` plan lens (round 2, R2-12, confidence 96). The fix this finding owes (descriptor-backed `AppDataDir` plus `mkdirat` for the leaf) must cover `EngineLaunch` as well as `db`, `engines`, `engine-images`, `puzzles` and `credentials`. Plan record: `tasks/handoffs/2026-09-14-macos-engine-launch-review.md`.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"3fddf89b2094c7304977555bc6de0f56517adeb503c9fdf4d8e7e064dea11a15","input_sha256":"eeba3060993c402de4b181ce6fe20f048219fb009c200de6c43a4445163e49ff","kind":"mutation-receipt","operation":"9fc0bb2136ad002d6fe363c6f85cb436f725b5c0614aa1a760639573ae2db342","options":{"section":null},"request_id_sha256":null,"results":["f-20260905-10"],"target":"f-20260905-10","v":1} -->
...
* **Closed:** Interactive `BestMovesPayload` is published only through `SupervisedEngine::try_publish`, which takes the same lock as `mark_cancelled`. Stop, kill (`terminate_exact`), actor replacement (`publish_admitted`), and `cancel_exact` all mark the generation cancelled before teardown. A UCI info/bestmove already dequeued after that mark is not emitted. A timed-out implicit stop now reaps the actor via `recover_failed_protocol` (same as Stop) so a later `go` cannot bind the old stream. Proof: `cargo test --manifest-path src-tauri/Cargo.toml --all-targets -- interactive_producer try_publish_skips terminate_exact_marks replacing_a_live_actor start_search_stop_timeout` (8 passed). Commit `e4e0f8d35f60aa6f4942ddf9c151145ccd6fc3f4`.
* **Rejected:** Switching the interactive loop to `next_line_cancellable` (that helper sends `stop_current` again while `stop_engine` already stop+terminates). Also rejected: leaving `terminate_exact` without `mark_cancelled` and relying on EvalListener's loaded-engine filter alone — Kill Engine never set the flag.
* **Left open:** `f-20260911-02` (`entry=build`) — unscoped `stop_generation` preferring a pending admission over the live actor. Named sibling; different mechanism and an open question. Lens r4 restated it as should-fix; already filed.
<!-- ledger-meta {"command":"annotate","effect_lines":3,"effect_sha256":"f411e4238fea7449f01580249b010010b335cbae770ea30e91b9a0f68ad76c67","input_sha256":"0a6a923e3e6cfac23c41c4c0fe06e8036ba91ea1824685ab8f4d0223532b6007","kind":"mutation-receipt","operation":"6cadc188954c9b5a0971ad64927998ab63fbaca45f555461f762bd096dc161be","options":{"section":null},"request_id_sha256":null,"results":["f-20260911-01"],"target":"f-20260911-01","v":1} -->
...
- **What:** agent-kit `f-20260907-04` (2026-09-12) shipped `findings.py merge-driver`, a git merge driver for the two append-only ledgers: two branches that each appended merge block by block and the union is validated with the receipt and structure checks of `check`; anything else falls back to `git merge-file`, so a real conflict stays a conflict. `findings.py check` installs the driver command into this clone's `.git/config` by itself. What this repo still lacks is the routing, two lines in `.gitattributes`:

  ```
  tasks/findings.md merge=ledger-append
  tasks/decisions.md merge=ledger-append
  ```

  Until they are in, a diverged branch with appends on both sides conflicts textually as before, and `kit check --consumers` in agent-kit fails for this repo (`.gitattributes does not set 'tasks/findings.md merge=ledger-append'`). Git's built-in `merge=union` was measured and rejected in agent-kit: it folds the common first line of two appended effect blocks and leaves the second receipt covering the wrong lines. Keep the existing line-ending attributes; append the two lines.

- **Proof:** `grep -cE '^tasks/(findings|decisions)\.md .*merge=ledger-append' .gitattributes` prints `2`; `./scripts/findings.py check` then `git config --get merge.ledger-append.driver` prints `./scripts/findings.py merge-driver %O %A %B %P`; `bash ~/Projekte/agent-kit/bin/kit check --consumers` no longer names this repo. Contract: `~/.claude/references/findings-ledger-contract.md`, "Merging diverged ledgers".

### `check-rust-release-surface.mjs` counts only path-qualified filesystem calls, so method-call reaches like `path.canonicalize()` are invisible

* **ID:** f-20260912-03 · **Status:** open · **Area:** gate-scripts · **Root:** - · **Entry:** build · **Blocked:** none
* **Where:** `scripts/check-rust-release-surface.mjs` — `qualifiedFsCall` (`:472`), `collectFilesystemMatches` (`:483-557`), `FS_FN_NAMES` / `PATHNAME_FNS` (`:210-236`); uncounted production sites measured on 2026-09-12: `src-tauri/src/db/search.rs:205` (`resolve_database(...)?.canonicalize()?`), `:616` (`file.canonicalize()?`), `src-tauri/src/main.rs:182`, `:194`, `:389` (`.canonicalize()`), and `src-tauri/src/db/mod.rs:163` (`path.metadata()` beside the counted `File::open`).
* **Defect:** R3 matches `std::fs::<fn>` / `<alias>::<fn>` / imported free functions by name, and R4 a fixed list of three `infra` pathname functions. `Path::canonicalize`, `Path::metadata`, `Path::exists`, `Path::read_dir`, `Path::symlink_metadata` and `Path::read_link` are inherent methods that reach the filesystem by pathname and match none of those shapes, so a file can carry pathname reaches while the checker reports zero. `src-tauri/src/db/search.rs` is not on the allowlist today and passes with two such reaches.
* **Why it is a design question:** a token-level checker cannot know a receiver's type, so `.metadata()` on an `fs::File` (descriptor-relative, wanted) and on a `Path` (pathname, the reach) look identical. Options: (a) match the method names and require an `#[allow]`-style local marker for descriptor receivers; (b) move the rule to a `clippy`/`dylint`-style typed lint; (c) accept the gap and document it. The choice decides whether the "empty allowlist" convention (`f-20260830-23`, `f-20260901-01`) is a measured property or a nominal one.
* **Open question:** which detection mechanism — name-based with a receiver marker, a typed lint, or a documented gap — carries the pathname-reach convention once the allowlist is empty?
* **Why it matters:** the allowlist ratchet is the gate that proves `native-fs` findings closed; a blind spot in it means a closed finding can be reopened by one method call nobody counts.
* **Related:** `f-20260830-23` (owner of the allowlist), `f-20260901-01`, `f-20260905-03` (whose locate stage found this).
* **Found by:** Claude Code, locate stage of `tasks/plans/2026-09-12-repository-identity-key.md`, 2026-09-12.
...
* **Why it matters:** the import is the longest native operation the app runs and the one the user is most likely to abort; the rule file lists a cancel flag checked after the emit as the incident class.
* **Related:** `f-20260904-05` (native cancellation, handled: cooperative SQL/PGN/CPU cancellation reached the other operations), `f-20260905-03` (whose plan review surfaced this).
* **Found by:** Codex `review-pgn-index` lens during plan review of `tasks/plans/2026-09-12-repository-identity-key.md`, 2026-09-12.

### The authority registry stores only the leaf identity of a file entry, so a parent directory swapped for one holding a hard link to the same inode passes every resolution

* **ID:** f-20260912-05 · **Status:** open · **Area:** native-fs · **Root:** - · **Entry:** build · **Blocked:** none
* **Where:** `src-tauri/src/infra/path_authority/mod.rs` — `StoredEntry` (identity of the leaf only), `validate_target` (`:1780-1799`), `workspace_entry_path` (`:4785-4813`), `retained_workspace_target` (`:4843-4870`), `database_file_target` (`:3626-3652`); `src-tauri/src/infra/fs.rs::open_verified_parent` (`:1170-1216`), which opens the parent by pathname and verifies only the leaf's `(dev, ino)` inside it.
* **Defect:** every resolution of a persistent file entry re-validates the leaf's `(dev, ino)` against the stored identity and then retains the parent directory reached through the pathname walk at that moment. Nothing records which directory the leaf was authorized in. If an ancestor with write access to the grandparent replaces the parent directory by one that contains a hard link to the authorized inode, the leaf check passes, the retained `parent` descriptor is the replacement directory, and every descriptor-relative sidecar write (`.ecsi` search index, SQLite `-wal`/`-shm`, atomic-replace temp names, unlink of sidecars) lands in the replacement directory while still "verified". The same holds for `retained_workspace_target`'s PGN mutation path.
* **Open question:** should `StoredEntry` carry the parent directory's identity (captured at registration through the same verified walk) so that `open_verified_parent` can refuse a parent whose identity differs, and what does that do to registrations whose parent legitimately changes identity (a moved or restored directory) — refuse, re-register through the dialog, or accept with a warning?
* **Why it matters:** the authority's contract is "descriptor-relative after resolution"; without a parent identity that contract is only as strong as the pathname walk at resolution time, which is the property the descriptors were introduced to remove.
* **Related:** `f-20260905-03` (its plan review surfaced this; the repository re-key compares the walked parent against the parent retained at mint, which bounds the window to before minting but cannot reach the registry); `f-20260905-07` (`AuthorizedDir`, `d-20260905-07`); `f-20260830-09`.
...
* **Annotated 2026-09-13 (plan review r4 of `tasks/plans/2026-09-13-workspace-directory-enumeration.md`, `f-20260905-05`, `review-root-cause`):** the descriptor-based workspace listing that run adds registers each PGN and directory with the identity observed through the enumerated directory's descriptor, but `persist_workspace_child` still stores `root + components` plus that leaf identity only. If `sub/` is replaced after enumeration by a fresh directory holding a hard link to the same `a.pgn`, the listing reads the trusted sidecar from its held descriptor, yet the registered `sub/a.pgn` handle later resolves through the replacement parent — this finding's state, reached through a new producer. That run leaves the registry model unchanged and names this finding as the owner.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"8c95528f5fa38b6985da3bc1b2642241b88cbaa80103085b7dffa7a92a38c4de","input_sha256":"9c375f34973767aff1c503c771d074d0430990e25a4489d9111bdd89c47361f5","kind":"mutation-receipt","operation":"0cfba80a45ced06956dddd7991a67d6b560c917cd40eb9fd6bf02f4fc40d5214","options":{"section":null},"request_id_sha256":null,"results":["f-20260912-05"],"target":"f-20260912-05","v":1} -->

* **Inherited review history (2026-09-13):** load `tasks/handoffs/2026-09-13-workspace-directory-enumeration-review.md` before this finding's plan review. It carries the `f-20260905-05` plan-review record; this finding inherits issue IDs W57 from it, with their witnesses, dispositions and evidence.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"7e0a8574c929960887ed7676c2fcb30805136c866db68fa2fafadde37eee60ab","input_sha256":"941938849eec6ea7673620d401911e746bf5309396ea6586ef99b53f34b339db","kind":"mutation-receipt","operation":"6c4513983a19fd1f4814328beafb99c616b9d1657db100d7dbc214405f723b8b","options":{"section":null},"request_id_sha256":null,"results":["f-20260912-05"],"target":"f-20260912-05","v":1} -->

### `data_revision` is not retained across LRU eviction, so a WAL-only import can leave a revision-0 search-index sidecar accepted as current

* **ID:** f-20260912-06 · **Status:** handled · **Area:** db-search · **Root:** - · **Entry:** build · **Blocked:** none
* **Where:** `src-tauri/src/db/repository.rs` — `EntryState.data_revision` (`:168-172`), `evict_idle_entries` (`:646-670`), `entry()` (`:559-604`, a fresh entry starts at `EntryState::default()`); consumers `IndexSource::from_database_identity` (`src-tauri/src/db/search_index.rs:236-250`) and `open_valid_preferred` / `archive.source() == expected` (`src-tauri/src/db/search.rs:312-341`).
* **Defect:** the in-process invalidation sequence lives only in the entry. When the LRU (`MAX_OPEN_DATABASES = 16`) evicts an idle entry, the revision is discarded; the next `entry()` for that database starts again at 0. A `.ecsi` sidecar written at revision 0 whose database then received a WAL-backed import that left the main file's length and mtime unchanged (a long-lived reader keeps the WAL uncheckpointed) is accepted again after the eviction, because `IndexSource` compares object, revision, length and mtime and all four match — the imported games are silently missing from search until something else changes the file.
* **Open question:** where does the revision live once an entry is gone — a bounded per-identity map in the repository that survives eviction, a persisted counter in the database's `info` table (`update_info_count` already exists in `db/mod.rs`), or a change to `IndexSource` that no longer relies on an in-process counter (which is `f-20260905-04` part one's territory)?
* **Why it matters:** the revision is the only invalidation the sidecar has against writes that do not move the main file's length or mtime; losing it on eviction makes the search index silently stale.
...
* **Why it matters:** search results are the product; a stale index answering for a replaced file is silently wrong data, not an error.
* **Related:** `f-20260905-03` (its plan review surfaced this; the repository re-key does not change the loader's sequence), `f-20260905-04`.
* **Found by:** Codex `review-pgn-index` lens during plan review of `tasks/plans/2026-09-12-repository-identity-key.md`, 2026-09-12.
...
* **Why it matters:** a read capability must not mutate, and the puzzle list is opened for every database the user has registered.
...
* **ID:** f-20260912-10 · **Status:** open · **Area:** native-fs · **Root:** - · **Entry:** build · **Blocked:** none
* **Where:** `src-tauri/src/infra/path_authority/mod.rs:3625-3652` (`#[cfg(unix)] pub(crate) fn database_file_target`), called without a platform gate from `src-tauri/src/db/mod.rs:2196-2200` (`delete_database_blocking`) and `src-tauri/src/db/search.rs:299-310` (the private `database_file_target` helper, itself ungated, used at `:206`, `:229`, `:279`); `src-tauri/src/infra/path_authority/mod.rs:4843` (`#[cfg(unix)] retained_workspace_target`) and its `#[cfg(unix)]`-only `workspace_mutation_target`; `.github/workflows/release.yml:17-27` (a Windows platform in the release matrix); `.github/workflows/test.yml:16` (tests run on `ubuntu-latest` only).
* **Defect:** the descriptor-relative authority surface was added under `#[cfg(unix)]` with `#[cfg(not(unix))]` refusals inside `infra/`, but two production callers outside `infra/` reference the unix-only method with no gate at all, so a non-unix `cargo check` fails to resolve `PathAuthority::database_file_target`. No gate measures this: CI compiles on Linux only, and the release workflow is the first place a Windows build would be attempted. `search.rs:343-348` shows the intended pattern (`#[cfg(not(unix))]` → `Conflict("… unsupported on this platform")`) applied to one function while its neighbours were left ungated.
* **Open question:** is Windows a supported target of this fork — in which case the descriptor APIs need `#[cfg(windows)]` implementations (`open_windows_child`, `windows_file_identity` and `resolve_windows` already exist at `mod.rs:1637-1700` and `resolved.rs:730`) and a non-unix `cargo check` in `test.yml` — or is it not, in which case the release matrix entry and the Windows branches in `infra/` are dead surface to remove? Either answer wants a compile gate so the next ungated caller is caught in CI.
* **Why it matters:** a release target that cannot compile is a promise the repository cannot keep, and every further descriptor-based change (this run's included) widens the gap silently.
...
* **Where:** `src-tauri/src/infra/path_authority/mod.rs:129-136` (`DatabaseFileTarget { parent, leaf, identity }`, `pub(crate)` fields), `:3605-3652` (`database_path` and `database_file_target`, the latter `DatabaseRead | DatabaseMutate` only), `src-tauri/src/db/mod.rs:196-210` (`resolve_database` → `database_path`), the call sites that obtain both a pathname and a target for one handle (`src-tauri/src/db/search.rs:205-206`, `:229`, `:279`; `src-tauri/src/db/mod.rs:2196-2200`), `src-tauri/src/infra/fs.rs:466-504` (`open_dir_no_follow` refuses every symlinked ancestor, so `database_file_target` fails today for a database under a symlinked directory while `database_path` serves it).
* **Defect:** the retained-descriptor carrier and the pathname the repository needs are minted by two different authority calls, validated at two different instants, and only one of them (`database_file_target`) refuses symlinked ancestors. Callers therefore hold two resolutions of one handle, the repository re-derives a canonical pathname from the loose `PathBuf` (`canonical_database_path`, the reach `f-20260905-03` removes), and a database whose ancestor is a symlink cannot be deleted or indexed although it can be read.
* **Fix shape (reviewed to convergence in `tasks/plans/2026-09-12-repository-identity-key.md`, rounds 1-5 of the rewritten series; this mandate is its "Mandate A"):** `DatabaseFileTarget` gains a private canonical `path: PathBuf` and `pub(crate)` accessors `parent()`, `leaf()`, `identity()`, `path()`; one module-private `assemble` and one `canonical_binding(path)` = `canonicalize(parent_of(path)).join(file_name)` (parent canonicalized inside `infra/`, leaf appended unchanged so a leaf symlink is never followed; empty parent means `"."` as `infra/fs.rs:463-465`); `database_file_target` accepts `DatabaseRead | DatabaseMutate | DatabaseCreate | DatabaseExport` and, after today's `validate_target(&stored, PersistentFile) == stored identity` check, opens the verified parent from the canonical binding; `resolve_database` returns the target and its callers use `target.path()` where they used the `PathBuf` (the repository still takes `&Path` in this mandate); `PathAuthority::database_path` is deleted (its only caller); `db/search.rs`'s private `database_file_target` helper is deleted and both source-text tests (`db/search.rs:1131-1140`, `db/mod.rs:3247-3258`) updated; the six literal constructions in `db/mod.rs`'s tests become `#[cfg(test)] DatabaseFileTarget::for_test_path(path)` (creates the file if absent, canonical binding, `open_verified_parent`). Tests: a `create_database_child` handle mints a `DatabaseCreate` target with the created inode and the canonical root path; a database registered through `dir/link/x.db3` (`link` → directory) mints with `path == dir/real/x.db3` — today refused; a registered leaf replaced before minting by a symlink to the moved original is refused; `for_test_path` on an absent and on a bare relative path. Proof: `cargo test --locked`, clippy `-D warnings` all targets, `cargo fmt --check`, `pnpm rust:surface:check` (allowlist unchanged: the repository's own reaches leave in Mandate B).
* **Open question:** none left for the carrier itself — the design converged; `build` because `infra/path_authority/**` and `db/**` are sensitive paths and the change spans the authority and every database command.
* **Why it matters:** it is the half of `f-20260905-03` that has no open design question; taking it first gives the repository re-key a carrier to key on and makes symlinked-ancestor databases usable by every operation.
...
* **ID:** f-20260912-12 · **Status:** handled · **Area:** native-fs · **Root:** - · **Entry:** build · **Blocked:** none
* **Where:** `src-tauri/src/db/mod.rs:196-210` (`resolve_database` → `PathAuthority::database_path`), its sixteen remaining callers (`:667`, `:991`, `:1067`, `:1103`, `:1151`, `:1327`, `:1565`, `:1671`, `:1713`, `:1821`, `:1987`, `:2387`, `:2449`, `:2596`, `:2726`, `:2779`, `:2911`), `src-tauri/src/db/search.rs:614` and `:840` (`resolve_database(...)` then `.canonicalize()`), `src-tauri/src/infra/path_authority/mod.rs:3605-3623` (`database_path`).
* **Defect:** once the carrier carries the canonical binding (Mandate A1 of `f-20260905-03`, 2026-09-12), `database_path` is a second, pathname-only resolution of the same handle with a different symlink policy, and every command that goes through `resolve_database` still holds a pathname the repository re-canonicalizes. The two resolvers should become one: `resolve_database` returns `DatabaseFileTarget`, every caller uses `target.path()` where it used the `PathBuf`, `database_path` and `db/search.rs`'s private `database_file_target` helper are deleted, and the search layer's own `canonicalize()` calls at `search.rs:614` and `:840` become `target.path()` (the binding is already canonical; a second pathname walk after minting can follow a leaf later swapped for a symlink).
* **Fix shape (reviewed in `tasks/plans/2026-09-12-database-target-canonical-binding.md`, rewritten series rounds 1-5):** the consumer migration above; source-text assertions that `fn database_path(` is absent from the production slice of `path_authority/mod.rs`, that `db/mod.rs` and `db/search.rs` contain no `DatabaseFileTarget {` literal and no `assemble(` call outside `mod tests`, and that `search_position_blocking` / `is_position_in_db_cancellable` contain no `canonicalize(`; a `commands_resolve_with_their_own_operation` test over a schema-initialized database whose handle is promoted with exactly one operation at a time — a `DatabaseRead`-only handle makes every non-read caller fail with the authority's exact "workspace entry does not permit this operation" and every read caller (`get_db_info`, `get_games`, `get_latest_game_timestamp`, `get_player`, `get_players`, `get_tournaments`, `get_players_game_info`, `search_position`, `is_position_in_db`) succeed; `DatabaseMutate`-only, `DatabaseCreate`-only and `DatabaseExport`-only handles make exactly their own callers pass the authority — anchored by a count of the `resolve_database(` sites; a directory-registered handle is refused before any filesystem access (the directory removed before minting). Proof: `cargo test --locked`, clippy `-D warnings` all targets, `cargo fmt --check`, `pnpm rust:surface:check` (allowlist unchanged), backend-test and backend-coverage receipts, `pnpm gates:contract:check`.
* **Open question:** none for the design; `build` because `infra/path_authority/**` and `db/**` are sensitive paths, and the operation matrix is the one test the plan review could not settle in five rounds (which callers to cover and how to distinguish the three non-read operations) — the shape above is what the last round asked for.
...
* **Unblocked, 2026-09-12:** A1 (`f-20260912-11`) landed in `70e17808`, so this entry's sequenced blocker is cleared. A2 can mint from the carrier that now carries `path()`.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"86160fe6809fa5262441064165021c65b011783600bcf87dac86a63f39d8f7c3","input_sha256":"dd4d3eb1aada3db56cbfa95d08a67d15d0b2c5981dd1caaaeaa856da20c50f94","kind":"mutation-receipt","operation":"e45f758ed1c5f8c08b42ab097d32b8a31234913b88cb625f502f27b67c4aefe8","options":{"section":null},"request_id_sha256":null,"results":["f-20260912-12"],"target":"f-20260912-12","v":1} -->
...
* **Inherited review history (2026-09-13):** load `tasks/handoffs/2026-09-13-workspace-directory-enumeration-review.md` before this finding's plan review. It carries the `f-20260905-05` plan-review record; this finding inherits issue IDs W20, W40, W63 and the category part of W98 from it, with their witnesses, dispositions and evidence.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"791ef26331135bcc3f5f0937f4cfd671b39be17b60cc7a43561ff0fab0c1c7eb","input_sha256":"2f297bc3a8e9e0a9c6c4d3a2a4301745e3fb121c652ce654aa61182dcdf73c2c","kind":"mutation-receipt","operation":"373bf47b9b9ef43469a7f024728228a27efeb74ae80a011444299ecfabcd174f","options":{"section":null},"request_id_sha256":null,"results":["f-20260913-04"],"target":"f-20260913-04","v":1} -->
...
* **Annotated 2026-09-13 (plan review r9 of `tasks/plans/2026-09-13-workspace-directory-enumeration.md`, `f-20260905-05`, `review-error-handling`):** a second accumulation belongs to the same bound question. A listing that fails after registry writes — a pass-2 registration or durability failure, cancellation between registrations, a later game-count failure in `list_file_workspace_core`, or a `.db3` listing failing after some children were registered — returns `Err` and leaves the handles it had already persisted; `commit_candidate` adopts a candidate before `require_durable` reports uncertainty, so the failing entry can remain too. Repeated failed listings of a changing tree therefore add fresh identities until `MAX_AUTHORITY_IDS`. The `f-20260905-05` plan keeps today's per-entry registration semantics for these failures; whether listings reconcile or prune what a failed call registered is part of this finding's bound.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"68776d7448378a8cafafa341b728e71e89a4975ef19e1e34fbc6f95e7932b111","input_sha256":"9ed9b740b2e4277c6a80b5f911cf4edacb43bc4c0720ca033c285a31406eed9a","kind":"mutation-receipt","operation":"662b830921235631799240cc67d961b16b55fd7670876b3ae61ee01bd5b2b57f","options":{"section":null},"request_id_sha256":null,"results":["f-20260913-06"],"target":"f-20260913-06","v":1} -->
...
* **ID:** f-20260914-01 · **Status:** handled · **Area:** db-search · **Root:** - · **Entry:** inline · **Blocked:** none
...
* **ID:** f-20260914-31 · **Status:** handled · **Area:** engine-uci · **Root:** non-linux-platform-port · **Entry:** build · **Blocked:** none
...
* **Handled 2026-09-15 (macOS engine-launch build run):** macOS now launches engines from pinned descriptors. Every launch flow (interactive analysis, report analysis, `get_engine_config`, game engines) goes through `resolve_launch`, which admits first, resolves leases under the path-authority lock on the blocking gateway, collapses duplicate options last-wins, and clones the executable and every effective file resource from the held, authorised descriptor into `engine-launch/<uuid>` (0o700, owned by an `flock`), with `fclonefileat` and a `pread` copy fallback on `EXDEV`/`ENOTSUP`. File resource values are the pinned leaf paths; directory resources are sent as `getpath(held)` and verified against the actor's held leases before any `setoption`. Linux keeps procfs values. The shell harnesses named in `Where` run on every Unix target and fall back to the plain value when it is not a procfs path; `inherited_descriptors_cover_every_resource_lease_and_the_engine_image` stays Linux-only because it tests the Linux inherited-descriptor mechanism itself, and macOS has its own pinned-leaf and directory-verification tests. Decision `d-20260915-02`; review record `tasks/handoffs/2026-09-14-macos-engine-launch-review.md` (16 plan rounds, 7 diff rounds, 3 post-push CI rounds). Code `3878a6a9`, `1e884b2b`, `ba539255`, `20d69da4`, `1ad9513e`, `42dbac5b`, `638839dc`, `dbcabea6`, `c8aaef6c`, `98124bf1`, `6ecd1ac8`, `6ae152e2`. Proof: CI run 34974674436 on `d0c698ff`, `rust-macos-test` green, with the nine Unix tests of this finding's original failure list (the tenth, `inherited_descriptors_cover_every_resource_lease_and_the_engine_image`, is now Linux-only by design; `inherited_resource_fd_survives_path_replacement_for_uci_child` runs as `authorized_resource_survives_path_replacement_for_uci_child`) and every new macOS-runnable test in the range reporting `ok`, including `report_core_restores_child_resource_provenance_after_fresh_resolution`, whose intermittent macOS failure exposed the search-read race fixed in `6ecd1ac8`.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"51cd5f0b30cc87c59e2504973f35f5bac95d424680cea29c98e37beb81da03ee","input_sha256":"d0c93921ee2690777896fe31150481e378b8c750a80e06b0b7753b732edbb013","kind":"mutation-receipt","operation":"10e28487b5116fa2cbc6faba81ea9e16f6bebd7d2fd108bc090e19a2a689784f","options":{"section":null},"request_id_sha256":null,"results":["f-20260914-31"],"target":"f-20260914-31","v":1} -->

### On macOS, listing a directory removed during enumeration returns success instead of `NotFound`

* **ID:** f-20260914-32 · **Status:** handled · **Area:** native-fs · **Root:** non-linux-platform-port · **Entry:** build · **Blocked:** none
...
* **Handled 2026-09-15 (macOS engine-launch build run):** one shared post-walk check, `ensure_directory_not_removed`, now runs after the walk of each held directory in `read_directory_entries_at`, `sync_tree` and `remove_tree_at`. Non-macOS Unix keeps `st_nlink == 0`; macOS compares `getpath(dir)` with `fstat(dir)` by device and inode (`held_matches_path`), treating `ENOENT`, `ENOTDIR` or a mismatch as removed, while a directory renamed away stays live. Listing keeps cancellation precedence; install refuses a source removed mid-walk; recursive delete reports the removal. This answers the open question for the recursive-delete and install walks as well. Decision `d-20260915-01`; review record `tasks/handoffs/2026-09-14-macos-engine-launch-review.md`. Code `506891a8` (plus the `target_os = "macos"` alignment in `ba539255`). Proof: CI run 34974674436 on `d0c698ff`, `rust-macos-test` green, with the originally failing `read_directory_entries_at_refuses_a_removed_directory` and every new removal, replacement and install/recursive-delete walk test reporting `ok`.
<!-- ledger-meta {"command":"annotate","effect_lines":1,"effect_sha256":"6f0f0c01a0a30da86a282645d59c1771500366335618ee6cae06d0bfcb92dbc0","input_sha256":"50ff7e71b7e6183392fcb6f8ee45aa95601cc3995f1f4dbff38048d3178e6e47","kind":"mutation-receipt","operation":"6bab363c38ce79c487458c888a6bd1d75581f0bef872ab5eb31670c03977af5f","options":{"section":null},"request_id_sha256":null,"results":["f-20260914-32"],"target":"f-20260914-32","v":1} -->

---

## 2026-09-14 — filed through the inbox spool

### A dialog selection under a symlinked ancestor is granted and then refused at promotion

* **ID:** f-20260914-33 · **Status:** handled · **Area:** native-fs · **Root:** non-linux-platform-port · **Entry:** build · **Blocked:** none
* **Where:** `src-tauri/src/infra/path_authority/mod.rs` `grant_dialog_operations` (stores `NativePath::from_path(path)` unchanged after `validate_dialog_target`, which uses `fs::symlink_metadata` and therefore follows ancestor symlinks), `promote_dialog` → `validate_target` → the no-follow ancestor walk `open_dir_no_follow` in `src-tauri/src/infra/fs.rs`; production callers `issue_file_workspace_blocking` (`src-tauri/src/file_workspace.rs`), `issue_pgn_workspace_blocking` (`src-tauri/src/main.rs`), and the in-authority grant inside the PGN creation path (`mod.rs`, `self.grant_dialog_operations(` near the `canonical_operations(EntryPurpose::PgnFile)` call).
* **Defect:** the grant step accepts a pathname whose ancestor is a symlink, but every later step walks the stored pathname with `O_DIRECTORY|O_NOFOLLOW` and refuses it with `ENOTDIR`. A user who picks a file or folder through a symlinked directory (macOS `/tmp` and `/var`, a symlinked home or data directory on Linux) gets a grant that can never be promoted or used. Observed on the real macOS runner in CI run 34862122222 (`rust-macos-test`, commit `8b98a870`): about 370 tests failed with `Io(NotADirectory)` at `promote_dialog` because `tempfile::tempdir()` lives under the `/var` symlink; the CI job now sets `TMPDIR=$RUNNER_TEMP` (commit `6544ca56`), which hides the fixture symptom but not the production path.
* **Why it matters:** the authority already binds legacy database paths to their canonical parent (`canonical_binding`, test `database_file_target_binds_a_symlinked_ancestor_to_its_canonical_parent`), so the same user selection is accepted through one door and unusable through another. On macOS, where Felix decided on 2026-09-12 that the fork ships, symlinked system directories make this reachable from an ordinary file dialog.
